...
function Content() {
  ...
  const { result1, result2, result3, result4 } = location.state || {};
  ...
  useEffect(() => {
    ...
    if (result1 === false || result3 === false || result4 === true || localStorage.getItem("email")) {
      setLogged(true);
    }
    if (result2 === true) {
      setLogged(false);
      ...
      <div className="bg-black flex justify-center">
        {logged && (
          <div>
            ...
            <button
              className="text-lg font-semibold text-white px-6 rounded m-2 bg-green-400"
              onClick={() => {
                if (show) {
                  setShow(false);
                } else {
      ...
      <div className="bg-black  flex justify-center content-center p-10 overflow-hidden">
        <div className="flex items-center flex-col  text-left mx-10">
          {jokes
            ...
            .filter((joke) => {
              if (show) {
                return joke.user === localStorage.getItem("email");
              } else {
                return jokes;
              }
            })
